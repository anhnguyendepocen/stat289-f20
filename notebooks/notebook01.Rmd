...
data. Let's just see a quick example for now. We will load a dataset called
...
Next, the following code uses the function `read_csv` to load this dataset into R,
...
You should also notice that the food dataset appears in the upper left-hand
corner of the screen. Clicking on it will open an Excel-like version of the
dataset. We will be working with these food data as a source of examples
...
in the dataset. We will learn more about these functions code over the
...
What are the observations in the dataset. **Answer**: Each observation is a city.

List three of the variables in the dataset. **Answer**: name, country, and population
are three examples.
