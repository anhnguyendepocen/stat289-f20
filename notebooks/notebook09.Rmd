...
to up a foreign key in one table with the primary key in another table, to add
new variables from one dataset into another dataset. Several examples will make
this notion more clear.
...
and `pescatarian` in the foods dataset. In order to do this, we want to put the
columns in the diet dataset into the food dataset in a way that is similar to
the `bind_cols` function. However, rather than "lining up" the rows, we can to
associate a row in the food dataset with the row in the diet dataset that
contains the same value of the variable `food_group`. The function to achieve
this is `left_join`; we pipe in the larger dataset that we want to join columns
...
In the code block below, I am ussing the `left_join` function to combine the
...
cities %>%
  ...
  ggplot() +
    geom_point(aes(x = lon, y = lat, color = continent_ka))
```
...
the character set you choose. They should run on the Linux server running
RStudio Cloud, however.
...
defaults given the long descriptions in the legend.
