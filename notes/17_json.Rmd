...
obj <- read_json(file.path("data", "apple.json"))
```
...
```{r, echo=FALSE}
json_text <- read_lines(file.path("data", "src", "wiki_pageviews", "apple.json"))
```
```{r, eval=FALSE}
...
The function `sm_check_download` can be used to check if a file exists locally,
...
in a directory called "wiki_parse".

```{r}
for (item in all_items)
{
  src_url <- paste0(base_url, options, item)  
  dest_path <- file.path("data", "src", "wiki_pageviews", paste0(item, ".json"))
...
df <- page_views_all %>%
  group_by(item) %>%
  summarize(sm_min(views), sm_max(views))
