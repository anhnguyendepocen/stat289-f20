...
to collect and organize data. Hadley Wickham's ["Tidy Data"](https://www.jstatsoft.org/article/view/v059i10),
...
principles for collecting datasets. The Data Carpentry guide
...
This chapter provides a summarized set advice for organizing and storing data
...
In Chapter \@ref(intror) the concept of a rectangular dataset, with observations
...
nonsense dataset with three variables:
...
impossible to work within R without first cleaning up the variable names after
loading the data.

I find the best approach to variable names is to only use lower-case letters,
numbers, and underscores. The underscores can be used in place of spaces, but
do not make variable names more complex than needed. Also, make sure to start
...
Throughout the rest of this chapter, continuing with the food theme, we will
...
For the specific variables within a dataset, spaces, capital letters, and
...
you later want to merge your dataset with other sources.
...
dataset with these principles applied:
...
Date variables are a well-known source of error in the collecting and recording
...
dataset. For example, here is a dataset showing properties of the cookbook
...
separate column approach while collecting your initial dataset. Later, if you
re-save a modified dataset *from within R*, the ISO 8601 format is a good
option.
...
to save your dataset in a plaintext format. The most commonly used type is
...
cookbook authors dataset looks like:
...
as a plaintext file. This is the best way for sharing and storing a dataset once
an analysis is finished, but if you are going to continue adding and changing
your dataset, it may actually be preferable to store your data in an `.xlsx`
...
package to read in a dataset, with either the first sheet or a named sheet:
...
your variables. To do this, we can construct a *data dictionary*. This should
...
or can be stored itself as a structured dataset. Here is an example of a data
dictionary for our authors dataset:
...
- create a data dictionary to record important information about your dataset
...
- **exercies07A.Rmd**: prompts for collecting a dataset from the web
- **exercies07B.Rmd**: prompts for collecting a dataset based on your life

For more information about collecting data, I recommend the following references:

- [R for Data Science: Chapter 12, Tidy Data](https://r4ds.had.co.nz/tidy-data.html)
- ["Tidy Data"](https://www.jstatsoft.org/article/view/v059i10)
- ["Data Organization in Spreadsheets"](https://www.tandfonline.com/doi/full/10.1080/00031305.2017.1375989)
- ["Data Carpentry guide"](https://datacarpentry.org/spreadsheet-ecology-lesson/02-common-mistakes/)

It will also be helpful to return to these notes after working through the
notion of wide and long data formats in Chapter 10.
