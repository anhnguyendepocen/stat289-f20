...
In recent years we have seen the creation of two different communities of R users.
The first focus on using many of functions and methods that exist in the core R language,
and the second have moved towards replacing these with a collection of R packages that
attempt to create more consistent set of functions that simplify the use of the language
for new users. This latter set of packages are known as the *Tidyverse*; the older set of
functions are often referred to as *Base R*.

In this class we will focus on using functions from the Tidyverse. This will be important
to know if you are trying to find help online.
