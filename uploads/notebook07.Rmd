...
observations in rows and variables in columns. This is the same format that
...
nonsense data set with three variables:
...
For the specific variables within a data set, spaces, capital letters, and
...
One thing to be careful of, particularly when using Excel, is that if your
computer is configured for a language that uses a comma as a decimal separator,
the default csv output may actually use a semicolon (`;`) in place of a comma.
To read these files in R, just replace the `read_csv` with the function
`read_csv2`.

Unlike some other sources, I am less strict about the need to only export data
as a plaintext file. This is the best way for sharing and storing a data set
once an analysis is finished, but if you are going to continue adding and
changing your data set, it may actually be preferable to store your data in
an `.xlsx` file (it avoids errors that are introduced when converting back and
forth between excel and plaintext formats). Data can be loaded directly from an
...
minimum, record the following variables:
