...
Take the plot from the previous question and change the date breaks and labels
to label each month with its full name.

```{r, question-02}

```

Now, we want to compare this to the number of deaths each day. However, the
`deceased` is a total count rather than the new number of deaths. Use the `lag`
function to add a `deceased_new` variable to the data from département 69:
...
What pattern do you see in the data? Do you have any concerns about the
reporting method used in France?
...
show the weekly change from Monday to Monday. Color the lines according the
direction of the change and try to include a good scale for the x-axis.
